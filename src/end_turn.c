		...
		switch (gBattleStruct->turnEffectsTracker) {
			...
			case ET_Aqua_Ring:
				...
				&& !IsHealBlocked(gActiveBattler))
				{
					...
					WishPearlRecoveryIncrease(gActiveBattler, &gBattleMoveDamage);
					gBattleMoveDamage *= -1;
			...
			case ET_Sleep:
				if (BATTLER_ALIVE(gActiveBattler) 
					...
					&& ABILITY(gActiveBattler) == ABILITY_FABULOUSDREAMS
					&& !BATTLER_MAX_HP(gActiveBattler)
					&& !IsHealBlocked(gActiveBattler))
				{
							...
							switch (itemEffect) {
								case ITEM_EFFECT_TOXIC_ORB:
									if (CanBePoisoned(gActiveBattler, gActiveBattler, FALSE))
									{
										...
										gBattleMons[gActiveBattler].status1 |= ((ABILITY(gActiveBattler) == ABILITY_GUTS || ABILITY(gActiveBattler) == ABILITY_VALOUR) ? STATUS1_POISON : STATUS1_TOXIC_POISON);
										EmitSetMonData(0, REQUEST_STATUS_BATTLE, 0, 4, &gBattleMons[gActiveBattler].status1);
										MarkBufferBankForExecution(gActiveBattler);
