...
void TryRevertOriginFormes(unusedArg struct Pokemon* mon, unusedArg bool8 ignoreDistortionWorld)
{
	...
	switch (GetMonData(mon, MON_DATA_SPECIES, NULL))
	{
		case SPECIES_GIRATINA_ORIGIN:
			if (itemEffect != ITEM_EFFECT_GRISEOUS_CORE
...
void HoldItemFormChange(struct Pokemon* mon, u16 item)
{
	...
	switch(species) {
		...
		case SPECIES_OGERPON_WELLSPRING:
			if (itemEffect == ITEM_EFFECT_OGERPON_MASK)
			{
				switch (type)
				{
					case TYPE_ROCK:
						targetSpecies = SPECIES_OGERPON_CORNERSTONE;
						break;
					case TYPE_FIRE:
						targetSpecies = SPECIES_OGERPON_HEARTHFLAME;
						break;
					case TYPE_WATER:
						targetSpecies = SPECIES_OGERPON_WELLSPRING;
						break;
				}
			}
			if (targetSpecies == SPECIES_NONE)
