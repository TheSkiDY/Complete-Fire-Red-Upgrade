	...
	if (WEATHER_HAS_EFFECT)
	{
		switch (defAbility) {
			...
			case ABILITY_MAGMAARMOR:
				if (gBattleWeather & WEATHER_SUN_ANY && atkAbility != ABILITY_INFILTRATOR && atkAbility != ABILITY_KEENEYE) //0.8 Magma Armor loss
					calc = udivsi((calc * 80), 100);
				break;

			case ABILITY_WATERVEIL:
				if (gBattleWeather & WEATHER_RAIN_ANY && atkAbility != ABILITY_INFILTRATOR && atkAbility != ABILITY_KEENEYE) //0.8 Water Veil loss
					calc = udivsi((calc * 80), 100);
				break;
