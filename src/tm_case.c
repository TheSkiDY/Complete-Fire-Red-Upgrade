...
/*
tm_case.c
	adds party icons to the tm case (Credits to Greenphx9)
*/
...
void PrintStringTMCaseOnWindow3(void)
{
    u32 distance = 78 - GetStringWidth(1, gText_TMCase, 0);
...
void ChangeMonIconPalsInTMCase(u16 itemId)
{
	...
	for (i = 0; i < MAX_SPRITES; ++i)
	{
		...
		if (sprite->oam.paletteNum < 6) //Is mon icon
		{
			...
			if (itemId != 0 && canLearn == 0) //Can learn move or already knows
			{
                sprite->oam.objMode = ST_OAM_OBJ_NORMAL;
				sprite->callback = SpriteCB_PokeIcon;
			}
			else if(itemId != 0 && canLearn == 2)
			{
                sprite->oam.objMode = ST_OAM_OBJ_NORMAL;
